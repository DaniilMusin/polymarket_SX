...
async def execute_arbitrage_trade(
    ...
    if not isinstance(size, (int, float)):
        raise ValueError(
            f"Size must be numeric: size={type(size).__name__}"
        )
    ...
    # Critical validation: exchanges must be different for arbitrage
    if buy_exchange_lower == sell_exchange_lower:
        raise ValueError(
            f"Invalid arbitrage: buy and sell exchanges must be different "
            f"(both are '{buy_exchange}'). This is not an arbitrage opportunity."
        )
